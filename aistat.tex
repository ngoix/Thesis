...
In a great variety of applications  (\textit{e.g.} fraud detection, distributed fleet monitoring, system management in data centers), it is of crucial importance to address anomaly/novelty issues from a ranking point of view. In contrast to novelty/anomaly detection (\textit{e.g.} \cite{Kolt97, VertVert, Scholkopf2001, Steinwart2005}), novelty/anomaly ranking is very poorly documented in the statistical learning literature (see \cite{VCTWMS} for instance). However, when confronted with
...
\item[(ii)]  For $t \ge 0$, we have for any $\epsilon > 0$, 
...
Assume that conditions $\mathbf{A_1}$, $\mathbf{A_2}$, $\mathbf{A_3}$ and $\mathbf{A_4}$ hold true, and that $f$ has a compact support. Let $\delta \in ]0,1[$, let
 $(t_k)_{k\in\{1,\;\ldots,\; K\}}$ be such that  $\sup_{1\leq k \le K}(t_{k}-t_{k+1}) = \mathcal{O}(1/\sqrt{n})$, setting $t_{K+1}=0$.
...
\begin{align*}
|EM^*(t)-EM_{s_K}(t)| ~~\le~~\left(A+\sqrt{2\log(1/\delta)}\right)\frac{1}{\sqrt n} ~+~ \lambda(t_{k+1})(t_{k}-t_{k+1}).
\end{align*}
\end{lemma}
...
\begin{remark}
Theorem~\ref{aistat:compact_support_case} holds true with the $t_k$'s defined as in \eqref{aistat:tk} -- even if the condition $\sup_{1\leq k \le K}(t_{k}-t_{k+1}) = \mathcal{O}(1/\sqrt{n})$ is not respected -- as soon as $t_K = \mathcal{O}(1/\sqrt{n})$.
\end{remark}
...
\end{align*}
where $o_N(1)=1-EM^*(t_N)$ represents `how much $f$ is heavy tailed'. In addition, $s_N(x)$ converges to $s_\infty(x):=\sum_{k=1}^{\infty} (t_{k+1}-t_k)\mathds{1}_{\hat \Omega_{t_{k+1}}}$ as $N \rightarrow \infty$ and $s_\infty$ is such that, for all $\delta \in (0,1)$, we have with probability at least $1-\delta$:
\begin{align*}
...
\begin{align*}
&\int_{\{f>t\}\setminus\{s>u\}}(f(x)-t)dx + \int_{\{s>u\}\setminus\{f>t\}}(t-f(x))dx\\
&~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\ge \int_{\{f > t + \epsilon\} \setminus \{s > u\} }(f(x) -t)dx +  \int_{\{s > u\} \setminus \{f > t + \epsilon\} }(f(x) -t)dx \\
&~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\ge \epsilon \leb(\{f > t + \epsilon\} \setminus \{s > u\}) + \epsilon \leb(\{s > u\} \setminus \{f > t + \epsilon\})
\end{align*}
...
\end{align*}
\noindent so that, with probability at least $(1-\delta)$, for $k \in \{1,\ldots,K\}$,
\begin{align*}
H_{n,t_k}(\hat \Omega_{t_k})-\Phi_n(\delta) ~~\le~~ H_{t_k}(\hat \Omega_{t_k}) 
 ~~\le~~ EM^*(t_k) 
~~\le~~ H_{n,t_k}(\hat \Omega_{t_k}) + \Phi_n(\delta) ~,
\end{align*}
\noindent whereby, with probability at least $(1-\delta)$, for $k \in \{1,\ldots,K\}$,
\begin{align*}

...
% the underlying quantity estimated here (if any). If we can define a scoring function on the input space whose restrictions to the random sub-spaces correspond to the scoring functions output by the algorithm, is there a 
%  and to study how fast is the convergence depending on the sub-sampling parameters. This would allow in particular to choose adaptively these parameters.

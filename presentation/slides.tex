...
\begin{block}{Solution: Excess-Mass curve}
\begin{itemize}
% \footnotesize{
...
\end{itemize}
\textbf{Property: Previous drawbacks are fixed with EM.}
\end{block}
...
In moderate dimension: [Thomas et al. 2015, 2016]
...
\small{
\begin{align*}
&\widehat{MV}_s(\alpha) = \inf_{u \ge 0} ~~~\leb(s \ge u) ~~~~\st~~~~ \mathbb{P}_n(s \ge u) \ge \alpha ~~~~~~~~\to \widehat{\crit}^{EM}(s)\\
&\widehat{EM}_s(t) = \sup_{u \ge 0} ~~~\mathbb{P}_n(s \ge u) ~-~ t \leb(s \ge u) ~~~~~~~~~\to \widehat{\crit}^{MV}(s)
\end{align*}
}
...
\begin{frame}
\frametitle{Heuristic extension for large dimension}

\begin{block}{Random projection and averaging} %~~~~{\color{black} \small [G. 2016]}}~\\
...
[Désir et al., 2013, Liu et al., 2008, Shi and Horvath, 2012].

\begin{block}{}{One-Class splitting criterion }%~~~~{\color{black} \small [G., Brault, Drougard, Chiapino 2016]}}
  
\input{tikz.tex}
\end{block}

Does theorical guaranties on RFs apply ?
[Biau et al. 2008, Biau and Scornet, 2016]

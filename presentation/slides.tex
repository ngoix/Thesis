...
%ex: {\blue EllipticEnvelope}
%-apply a statistical inference test to determine if a new instance belongs or not to this model.\\~\\
...
- {\blue Minimum Volume set} estimate {\small[Einmahl and Mason 92, Polonik 97, Scott and Nowak 2006]}\\~\\
...
\begin{minipage}{0.55\textwidth}
	\begin{center}
	{\blue Minimum volume set} [Polonik, 1997]
	\end{center}
	\begin{equation*}
	\Gamma^*_{\alpha} = \argmin_{\Gamma\text{ borelian} } \{\text{Leb}(\Gamma), \mathbb{P}(\mb X \in \Gamma) \geq \alpha \}
	\end{equation*}
\end{minipage}
...
%\max_{\Gamma\text{ borelian} } ~\big\{{\mathbb{P}} (\mb X\in \Gamma)-t_\alpha\leb(\Gamma) \big\}.$$

{\blue Mass Volume curve} of a scoring function $s$ [Clémençon and Jakubowicz, 2013]:
% \footnotesize{
\begin{align*}
...
\begin{frame}
\frametitle{Drawbacks and alternative criterion: Excess-Mass curve}

Drawbacks of MV:
\begin{itemize}
\item When optimized \emph{w.r.t.} different levels $\alpha$, produces not necessarily nested empirical level sets.
\item $\to$ low convergence rates -- of order $O(n^{-1/4})$.
\item MV diverges in $1$ in case of unbounded support.
\end{itemize}

\begin{block}{Excess-Mass curve}
\begin{itemize}
% \footnotesize{
...
\begin{align*}
EM^*(t) ~:=~ EM_f(t) = EM_{T \circ f}(t) = \max_{\Omega\text{ borelian} } ~\big\{{\mathbb{P}} (\mb X\in \Omega)-t\leb(\Omega) \big\}\\
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~\textbf{how {\blue large} is $EM_s$ on $I$ ?} ~~~~~~~~$\to$  {\red $\mathcal{C}^{EM} = \|EM_s\|_{1, I}$}  \\~\\~\\}
...
$\updownarrow$\\
~~~~~~~~~~~~~~~~~~~~~~~~~\textbf{how {\blue small} is $MV_s$ on $J$ ?} ~~~~~~~$\to$  {\red $\mathcal{C}^{MV} = \|MV_s\|_{1, J}$}
}
...
\item \textbf{Issue:}
\small{
The volume $\leb(s \ge u)$ has to be estimated (Monte-Carlo). Challenging in high dimensions.
}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.33]{sourcefigs/representation2Drect.png}
\end{figure}
...
\begin{minipage}{0.5\linewidth}
\centering
\includegraphics[scale=0.35]{sourcefigs/representation2D_nk_rect.png}
\end{minipage}
~\\
...
\begin{frame}
\frametitle{Statistical guaranties: Main issue}
...
$$\mb p \lesssim d \frac{k}{n}$$
$\Rightarrow$ bound in $$ d \sqrt{ \frac{1}{ k}\log \frac{1}{\delta}} $$
interpretation of $k$: 
\begin{itemize}
\item $k \propto$ number of data considered as extreme
\item $k \simeq$ number of data used for estimation
\end{itemize}
